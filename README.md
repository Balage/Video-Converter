...
### "Probe" tab
This one uses `ffprobe.exe` to display every possible media and codecs information in a file. There's no settings panel for this one.
### "Convert Video" tab
...
- Image Sequence Options
	...
	- `Generated frames`: This will add generated frames in-between the input images for a higher frame rate. Having a base frame rate of 30 with 1 generated frame will create a video at 60 FPS.
		- **Warning:** frame generation is very slow! You can use the `Terminate Process` button on the toolbar if it takes more time than expected.
	- `PNG background color`: Input images can be PNG, but the output video cannot have alpha channel, so this color will be used as background.
### Toolbar (top-right)
